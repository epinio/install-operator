...
func init() {
	...
	utilruntime.Must(epiniov1alpha1.AddToScheme(scheme))
...
func main() {
	...
	flag.StringVar(&uiAddr, "ui-bind-address", ":8082",
		"The address the install UI binds to. Use 0 to disable the UI server.")
	flag.StringVar(&installerServiceAccount, "installer-service-account", "",
		"Service account used by the installer Job. Defaults to the manager Pod service account.")
	flag.StringVar(&installerHelmImage, "installer-helm-image", "alpine/helm:3.18",
		"Container image used for the installer Job.")
	...
	if uiAddr != "0" {
		...
		if err != nil {
			setupLog.Error(err, "Failed to initialize UI server")
			os.Exit(1)
		}

		go func() {
			setupLog.Info("Starting install UI server", "address", uiAddr)
			if err := uiServer.ListenAndServe(uiAddr); err != nil {
				setupLog.Error(err, "Failed to run install UI server")
			}
		}()
		...
		defer func() {
			...
			if err := uiServer.Shutdown(ctx); err != nil {
				setupLog.Error(err, "Failed to stop install UI server")
			}
		}()
	}
